...
* [Slice Layers](#slice-layers)
  ...
  * [Listeners (Planned)](#listeners-planned)
...
OrcheStore provides multiple ways to access state depending on the
context.

| API                 | Purpose                                                  |
| ------------------------------- | -------------------------------------------------------- |
| `slice.getState()`              | Read the current state snapshot                          |
| `slice.getInitialState()`       | Read the slice's original initial state                  |
| `slice.getInitialState.deep()`  | Read the original initial state including child slices   |
...
Use it whenever you need to read state imperatively outside of React
subscriptions.
...
1. `getInitialState()` returns the original initial state used when the
slice was created.

``` ts
counter.getInitialState().value;
```

2. `getInitialState.deep()` extends the result to include the initial state
of all mounted child slices.

```ts
shop.getInitialState().products.items;
```
...
- Both APIs always return the original initial state and are
  **unaffected by later mutations**.
- `getInitialState()` returns only the slice's own initial state.
- `getInitialState.deep()` includes the initial state of every mounted
  descendant.

``` ts
methods: {
	resetName() {
		this.setName(this.getInitialState().name);
	}
}
```
...
`useSelect()` provides reactive state subscriptions for React
components.

Internally, it is powered by Redux Toolkit's `useSelector`, while
exposing a fully typed, slice-scoped API.
...
Selectors can also be accessed through the store to access multiple
slices.
...
Draft state provides temporary access to slice state during state
evaluation.
...
- Inside Computed State (Planned)

- Inside useSelect

Selectors receive a read-only draft state.
