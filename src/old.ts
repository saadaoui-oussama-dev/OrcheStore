...
const exposeContext = (type: string, slice: AnySlice) => ({ module: "createSlice", type, slice: slice.name });
...
export function createSlice<
	...
	M extends Methods,
	G extends Computed<S, R, M>,
	N extends string = string,
>(props: SliceOptions<S, R, M, G, N>): Slice<S, R, M, G, N> {
	...
	exposeLayer(exposeContext("mutation", slice), options.mutations, [reservedKeys, injectedKeys], (key, item) => {
	...
	exposeLayer(exposeContext("method", slice), options.methods, [reservedKeys, injectedKeys], (key, item) => {
	...
	// Exposing children
	exposeLayer(exposeContext("child", slice), options.children, [reservedKeys, injectedKeys], (key, item) => {
		if (!!true) return;
...
export const exposeSliceToParent = (name: string, childData: SliceData, parent: any, store: AnyStore, reducers: any) => {
	...
	childData.path = name;
	childData.exposedIn.push(store);
...
export function createStore<C>(props: { slices: C }): Store<C> {
